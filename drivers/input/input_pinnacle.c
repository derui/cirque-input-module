...
static bool pinnacle_handle_rounding_scroll(const struct device *dev, int16_t clamped_x,
    ...
    if (!data->in_rounding_scroll && z > 0) {
      ...
      uint16_t edge_width = 512 - sqrt(dx * dx + dy * dy);

      // when the finger is near the edge, start rounding scroll mode.
      if (edge_width < config->rounding_scroll_detection_edge) {
...
#define PINNACLE_INST(n)                                                                           \
    ...
    static const struct pinnacle_config pinnacle_config_##n = {                                    \
        ...
        .rounding_scroll_detection_edge = DT_INST_PROP(n, rounding_scroll_detection_edge),        \
